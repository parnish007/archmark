...
export function levenshteinCapped(a: string, b: string, cap = 3): number {
  ...
  const m = Math.min(a.length, 64); const n = Math.min(b.length, 64);
  const aa = a.slice(0, m); const bb = b.slice(0, n);
